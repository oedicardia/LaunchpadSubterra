...
# enabled
	...
	def _scale_updated(self):
		"""
		Recalculate all scale-dependent arrays based on current scale mode.

		CRITICAL CONSTRAINT: key_indexes[] MUST BE INCREASING (keys[0] < keys[1] < ... < keys[7])

		MODES:
		- RELATIVE: Root note ALWAYS at keys[7] (bottom row)
		- ABSOLUTE: Root note at its NATURAL scale position within the 8-note window
		"""
		...
		if hasattr(self, '_scale_selector') and self._scale_selector:
			if hasattr(self._scale_selector, '_octave'):
				octave = self._scale_selector._octave
			if hasattr(self._scale_selector, '_is_absolute'):
				is_absolute = self._scale_selector._is_absolute

		if hasattr(self._note_selector, '_key'):
			key_value = self._note_selector._key
			if key_value >= 12:
				root_midi_note = key_value
			elif hasattr(self._note_selector, '_root_note'):
				root_midi_note = self._note_selector._root_note
			else:
				root_midi_note = key_value + (octave * 12)
		else:
			root_midi_note = 36
		...
		if self._note_selector.is_drumrack:
			for i in range(8):
				keys[i] = getattr(self._note_selector, 'selected_note', 36) + i
				key_is_root_note[i] = (keys[i] + 12 + 16) % 16 == 0
				key_is_in_scale[i] = (keys[i] + 12 + 16) % 4 == 0

		# ========================================
		# CASE 2: DIATONIC MODE
		# ========================================
		elif self._note_selector.is_diatonic:

			scale = self._note_selector._scale
			scale_length = len(scale)

			root = self._note_selector._root_note
			root_note_class = root % 12

			# Number of rows ABOVE the root in absolute mode
			# C=0, D=1, E=2, F=3, G=4, A=5, B=6
			start_degree = 0 if not is_absolute else -NATURAL_ROW[root_note_class]
			...
			for i, note in enumerate(keys):
				key_is_root_note[i] = (note % 12) == root_note_class
				key_is_in_scale[i] = True

			if DEBUG_LOGGING:
				self._control_surface.log_message(
					"[SCALE_UPDATED] {} Keys={}".format(
						"ABSOLUTE" if is_absolute else "RELATIVE",
						keys
					)
				)

		# ========================================
		# CASE 3: CHROMATIC MODE
		# ========================================
		else:
			for i in range(8):
				keys[i] = root_midi_note - 7 + i

			for i in range(8):
				key_is_root_note[i] = (keys[i] % 12 == root_note_class)
				key_is_in_scale[i] = True

		# ========================================
		# UPDATE INTERNAL STATE
		# ========================================
		self._key_indexes = keys
		self._key_index_is_root_note = key_is_root_note
		self._key_index_is_in_scale = key_is_in_scale

		if DEBUG_LOGGING:
			is_ascending = all(keys[i] <= keys[i + 1] for i in range(len(keys) - 1))
			self._control_surface.log_message(
				f"[SCALE_UPDATED] ASCENDING CHECK - {'✓ PASS' if is_ascending else '✗ FAIL'} Keys={keys}")

			if not is_ascending:
				self._control_surface.log_message(f"[SCALE_UPDATED] ERROR: Keys are NOT ascending!")
		...
		if DEBUG_LOGGING:
			self._control_surface.log_message(
				f"[SCALE_UPDATED] COMPLETE - Mode={'ABSOLUTE' if is_absolute else 'RELATIVE'} Keys={keys}")

	def _build_keys_from_pattern(self):
		"""
        Build the 8 sequencer pitches directly from the Launchpad95
        MelodicPattern.

        This guarantees that Instrument Mode and the Step Sequencer
        always use exactly the same note layout.
        """

		pattern = self._scale_selector.get_pattern()

		keys = []
		root = []
		inscale = []

		horizontal = getattr(self._scale_selector, "_is_horizontal", True)

		#
		# Read one complete Launchpad column (or row if vertical)
		#
		for lp_row in range(8):

			if horizontal:
				info = pattern.note(0, lp_row)
			else:
				info = pattern.note(lp_row, 0)

			keys.append(info.index)
			root.append(info.root)
			inscale.append(info.in_scale)

		#
		# Sequencer expects low→high.
		#
		keys.reverse()
		root.reverse()
		inscale.reverse()

		return keys, root, inscale

	def build_diatonic_keys(scale,root_note_class,root_scale_index,root_midi_note,absolute_mode):
		"""
			scale:
				self._note_selector._scale
				already transposed by ScaleComponent
			"""
		scale_len = len(scale)
		# Which scale degree should appear on row 0?
		if absolute_mode:
			#
			# C -> start at degree 0
			# D -> start at degree -1
			# E -> start at degree -2
			#
			first_degree = -root_scale_index
		else:
			#
			# Root always on row 7
			#
			first_degree = 0
		root_octave = root_midi_note // 12
		keys = []

		for i in range(8):
			degree = first_degree + i
			octave = math.floor(degree / scale_len)
			note = scale[degree % scale_len] + octave * 12
			#
			# scale[] already contains the chromatic root.
			#
			midi = note + root_octave * 12
			keys.append(midi)
		return keys
