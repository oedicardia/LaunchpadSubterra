...
class Launchpad(ControlSurface):
	...
	def init(self):
		...
		with self.component_guard():
			...
			top_buttons[5].name = 'H_arpeggiator'
			top_buttons[7].name = 'Custom_drum_mode'
			top_buttons[6].name = 'Melodic_step_sequencer'
			...
			if self._lpx:
				self.log_message("LaunchPad_Subterra (LPX) Loaded !")
			elif self._mk3_rgb:
				self.log_message("LaunchPad_Subterra (mk3) Loaded !")
			elif self._mk2_rgb:
				self.log_message("LaunchPad_Subterra (mk2) Loaded !")
			else:
				self.log_message("LaunchPad_Subterra (classic) Loaded !")
